...
select * from countries,departments,employees,job_history,jobs,regions,locations;
Use Hr;
...
select concat(first_name, ' ', last_name) as employees_name, hire_date
from employees;

#9. Display the names and salaries of all employees in reverse salary order
select concat(first_name, ' ', last_name) as employees_name, salary
from employees
order by salary desc;

#10.Display 'ename" and "deptno" who are all earned commission and display salary in reverse orde
select concat(first_name, ' ', last_name) as "ename",department_id as "detno", salary
from employees
where commission_pct and salary is not null
order by salary desc;
...
where (job_id = 'SA_REP' OR job_id = 'ST_CLERK')
  ...
  #13 1) Display the maximum, minimum and average salary and commission earned. 
  SELECT 
...
sum(coalesce(commission_pct, 0) * salary) as total_commission_payout
from employees
...
select concat(first_name, ' ' , last_name) as employee ,department_id as department,
...
coalesce (commission_pct * 2, ' no commission') as updatecommission
from employees;

##18. Display the employee's name, department id who have the first name same as another
# employee in the same department.
select concat(first_name, ' ', last_name) as employees , department_id
from employees
...
SELECT concat(employees.first_name, ' ', employees.last_name) as manager_name,
	   count(subordinates.employee_id) as employee_count,
       departments.department_id as manager_department_id
from employees
join employees as subordinates on employees.employee_id = subordinates.manager_id
left join departments on employees.employee_id = departments.manager_id
group by  employees.employee_id, manager_name, manager_department_id;
...
GROUP BY manager_name, employee_name, d.department_id, e.salary;
...
# 23) Select the maximum salary of each department along with the department id 
select department_id,
max(salary) as maximaumsalary
from employees
group by department_id;

# 24) Display the commission, if not null display 10% of salary, if null display a default value 1
...
# 25. Write a query that displays the employee's last names only from the string's 2-5th
# position with the first letter capitalized and all other letters lowercase, Give each column an
# appropriate label.

SELECT
    employee_id,
    CONCAT(UPPER(SUBSTRING(last_name, 1, 1)), LOWER(SUBSTRING(last_name, 2, 4))) AS formatted_last_name
FROM
    employees;

#26 2. Write a query that displays the employee's first name and last name along with a " in
...
FROM
    ...
    #27 Write a query to display the employee's last name and if half of the salary is greater than
...
FROM
    ...
    #28 Display the employee ID by Appending two zeros after 2nd digit and 'E' in the end,
...
# 32. Display First names of all employees with initcaps. 
   SELECT
    CONCAT(
        UCASE(SUBSTRING_INDEX(first_name, ' ', 1)),
        ' ',
        UCASE(SUBSTRING_INDEX(last_name, ' ', 1))
    ) AS initcap_name
FROM
    employees;
...
#34. Extract first letter from First Name column and append it with the Last Name. Also add"@systechusa.com" at the end. Name the column as e-mail address. All characters should
# be in lower case. Display this along with their First Name.

SELECT
    first_name,
    LOWER(CONCAT(SUBSTRING(first_name, 1, 1), last_name, '@systechusa.com')) AS email_address
FROM
    employees;
    
    #35 11. Display the names and job titles of all employees with the same job as Trenna
    SELECT
    e1.first_name,
    e1.last_name,
    e1.job_id
...
FROM
    employees e1
JOIN
    locations l1 ON e1.location_id = l1.location_id
JOIN
    employees e2 ON l1.city = (SELECT l2.city FROM employees e3 JOIN locations l2 ON e3.location_id = l2.location_id WHERE e3.first_name = 'Trenna')
JOIN
...
#39 1. Write a query to display the last name, department number, department name for all employees.
...
SELECT
    e.last_name AS employee_last_name,
    e.department_id AS employee_department_number
FROM
    employees e
JOIN
...
WHERE
    ...
    #49 Write a query to display the last name and hire date of any employee in the same department as SALES. 
...
#50 Create a query to display the employee numbers and last names of all employees who earn more than the average salary. Sort the results in ascending order of salary
...
#51 Write a query that displays the employee numbers and last names of all employees who work in a department with any employee whose last name contains a' u 
 SELECT
    employee_id,
    last_name
FROM
...
##52 Display the last name, department number, and job ID of all employees whose department location is ATLANTA.
...
WHERE
    ...
    ##53 Display the last name and salary of every employee who reports to FILLMORE
    SELECT
    e.last_name,
    e.salary
FROM
    employees e
JOIN
    employees manager ON e.manager_id = manager.employee_id
WHERE
    manager.last_name = 'FILLMORE';
    
    ##54 Display the department number, last name, and job ID for every employee in the OPERATIONS department.
...
#55 Display the names of all employees whose job title is the same as anyone in the sales dept.
   SELECT
    e.first_name,
    e.last_name
FROM
...
#56 Write a compound query to produce a list of employees showing raise percentages, employee IDs, and salaries. Employees in department 1 and 3 are given a 5% raise,
...
#57 Write a query to display the top three earners in the EMPLOYEES table. Display their last names and salaries. 
...
#58 Display the names of all employees with their salary and commission earned. Employees with a null commission should have O in the commission column 
...
#60 Display the Managers (name) with top three salaries along with their salaries and department information.
 SELECT
...
JOIN
    departments d ON e.department_id = d.department_id
WHERE
    e.salary IS NOT NULL -- Exclude employees without salary information
ORDER BY
    e.salary DESC
LIMIT 3;
